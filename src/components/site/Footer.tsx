...
const cols = [
  { title: "Shop", links: ["Airguns", "Pellets", "Tactical Vests", "Gun Pouches", "Optics", "Hunting Gear"] },
    ...
    <footer className="bg-charcoal text-white/70">
      <div className="container-x pt-20 pb-10">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-10">
          <div className="col-span-2">
            <div className="flex items-center gap-2.5">
              <div className="grid h-9 w-9 place-items-center bg-accent text-accent-foreground rounded-sm">
                <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M3 21l9-18 9 18M7 17h10" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              <div className="leading-none">
                <div className="font-display text-xl text-white">RIDGELINE</div>
                <div className="text-[10px] tracking-[0.3em] text-white/40">OUTFITTERS · EST 1987</div>
              </div>
            </div>
            <p className="mt-6 text-sm leading-relaxed max-w-sm">
              Field-tested hunting and outdoor equipment for those who refuse to compromise.
              Family owned, three generations strong.
            </p>
            <div className="mt-8 flex gap-3">
              {[Instagram, Facebook, Youtube, Twitter].map((I, i) => (
                <a key={i} href="#" className="grid h-10 w-10 place-items-center rounded-full border border-white/15 hover:bg-accent hover:border-accent hover:text-accent-foreground transition">
                  <I className="h-4 w-4" />
                </a>
              ))}
            </div>
        ...
        <div className="mt-16 pt-8 border-t border-white/10 flex flex-col md:flex-row gap-4 items-center justify-between text-xs text-white/50">
          <div>© {new Date().getFullYear()} Ridgeline Outfitters. All rights reserved.</div>
