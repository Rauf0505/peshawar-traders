...
import { getCategories, deleteCategory } from "@/lib/api/products.server";
...
export function CategoryListPage() {
  ...
  const isChildRoute = location.pathname.startsWith("/admin/categories/");
  ...
  const toggleStatus = async (cat: any) => {
    const { updateCategory } = await import("@/lib/api/products.server");
    try {
    ...
    <div className="p-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 border border-dashed border-zinc-800 rounded-lg">
          ...
          <Link to="/admin/categories/new" className="inline-flex items-center gap-2 px-4 h-9 bg-emerald-600 hover:bg-emerald-500 text-sm font-medium rounded-md transition text-white">
